...
impl ProjectionExec {
    ...
    fn compute_properties(
        ...
        // carried over is the only thing reuse changes; everything below is
...
impl ExecutionPlan for ProjectionExec {
    ...
    fn with_new_children(
        ...
        // The macro above requires the child's properties to be unchanged
        // outright. A rule that introduces a sort below this projection does not
        // qualify, yet the child's *equivalence group* is still identical:
        // sorting changes which orderings hold, not which expressions are equal
        // to one another. Projecting that group again would reproduce the group
        // already cached here, so reuse it and derive only the orderings.
        // Hand over what this projection was built from and what that produced;
        // `project_reusing` decides whether the group can be carried over and
        // falls back to a full projection otherwise.
        let reuse_from = Some((
...
mod tests {
    ...
    #[test]
    fn test_with_new_children_reuses_eq_group_when_only_orderings_change() -> Result<()> {
        let child = filtered_source("a", "b")?;
        ...
        let replaced =
            Arc::clone(&projection).with_new_children(vec![Arc::clone(&sorted)])?;
    ...
    #[test]
    fn test_with_new_children_reuses_eq_group_across_a_nullability_change() -> Result<()>
    {
    ...
    #[test]
    fn test_with_new_children_recomputes_when_eq_group_changes() -> Result<()> {
        let child = filtered_source("a", "b")?;
        let exprs = renaming_exprs(&child.schema())?;
        let projection =
            Arc::new(ProjectionExec::try_new(exprs.clone(), Arc::clone(&child))?);
        ...
        let replaced =
            Arc::clone(&projection).with_new_children(vec![Arc::clone(&other)])?;
    ...
    #[test]
    fn test_with_new_children_keeps_properties_when_the_child_is_unchanged() -> Result<()>
    {
        // `check_if_same_properties!` compares the children's `PlanProperties`
        // by pointer, so handing back the very same child takes the existing
        // fast path ahead of the equivalence-group one. The cached properties
        // must then survive verbatim rather than being derived again.
        let child = filtered_source("a", "b")?;
        let exprs = renaming_exprs(&child.schema())?;
        let projection = Arc::new(ProjectionExec::try_new(exprs, Arc::clone(&child))?);

        let replaced = Arc::clone(&projection).with_new_children(vec![child])?;
